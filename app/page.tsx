...
function EditableBlock({
  ...
  return (
    <div
      ...
      onInput={(event) =>
        onChange(event.currentTarget.innerText.replace(/\s+$/, ""))
      }
      onKeyDown={(event) => {
...
export default function Home() {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 429) {
          throw new Error("您所在 IP 今日限额啦，次日更新～");
        }
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "PDF 生成失败，请重试。"
      );
    } finally {
      ...
      <div className="mx-auto flex max-w-[900px] flex-col gap-4 px-4 py-6">
        <div className="flex flex-wrap items-center justify-between gap-3 text-sm text-slate-600">
                ...
                {templates.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-2 rounded border px-2.5 py-1 text-xs transition ${
                        active
          ...
          <div className="flex items-center gap-3">
            <a
              href="https://github.com/qingning7/AI-resume-generator"
              target="_blank"
              rel="noreferrer"
              className="flex h-9 w-9 items-center justify-center rounded bg-slate-900 text-white transition hover:bg-slate-800"
            ...
            <div className={headerClass}>
              ...
              <div className={`flex flex-col gap-2 ${avatarWrapperClass}`}>
                <label
                  ...
                  className={`flex cursor-pointer items-center justify-center border bg-slate-50 text-xs text-slate-400 ${template.avatarFrameClass}`}
